...
impl FrameWriter for StdoutWriter {
	...
	fn write_frame(&mut self, timestamp: moq_net::Timestamp, payload: Bytes) -> Result<(), Self::Error> {
		...
		writeln!(stdout, "{}", hex::encode(&payload)).map_err(|error| Error::Io(Arc::new(error)))?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let inner = StdoutWriter { sequence_number: 0 };
	...
	writer.write_frame(moq_net::Timestamp::from_secs(2)?, Bytes::from_static(b"delta frame"))?;
