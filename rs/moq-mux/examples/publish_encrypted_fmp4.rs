...
use moq_mux::{container::fmp4::Import, encryption::moq_secure_adapter::MoqSecureEncrypter};
	...
	fn parse() -> Result<Self> {
		...
		while let Some(arg) = args.next() {
			match arg.as_str() {
				...
				"--relay" => {
					relay_url = args.next().ok_or_else(|| anyhow!("--relay requires a URL"))?;
				}

				"--broadcast" => {
					broadcast_name = args.next().ok_or_else(|| anyhow!("--broadcast requires a name"))?;
				}

				"--track" => {
					track_name = args.next().ok_or_else(|| anyhow!("--track requires a track name"))?;
				}
...
async fn main() -> Result<()> {
	...
	let encoded_aead_key =
		env::var("MOQ_AEAD_KEY").context("MOQ_AEAD_KEY must contain a 32-byte key encoded as hex or base64")?;
	...
	let signing_key = match env::var("MOQ_SIGNING_KEY") {
		Ok(encoded) => {
			let bytes = decode_32_bytes(&encoded).context("decoding MOQ_SIGNING_KEY")?;
	...
	let publisher_result = run_publisher(&origin, encrypter, &options.input, &options.broadcast_name).await;

	let _ = publisher_done_tx.send(true);

	publisher_result?;

	subscriber_task.await.context("subscriber task panicked")??;
...
async fn connect_to_relay(relay_url: &str) -> Result<MoqSession> {
	let origin = moq_net::origin::Producer::random();
	...
	let connection_task = tokio::spawn(async move { reconnect.closed().await.context("MoQ connection closed") });
...
async fn run_publisher(
	...
	let broadcast = origin.create_broadcast(broadcast_name).context("creating broadcast")?;

	let mut importer = Import::new(broadcast).with_encrypter(encrypter);
	...
	loop {
		let count = stdout.read(&mut buffer).await.context("reading fMP4 from ffmpeg")?;

		if count == 0 {
			break;
		}

		importer.decode(&buffer[..count]).context("decoding fMP4 fragment")?;
	}
...
async fn run_subscriber(
	...
	let broadcast = tokio::select! {
		result = consumer.routed_broadcast(&broadcast_name) => {
			result
				.with_context(|| {
					format!(
						"resolving broadcast `{broadcast_name}`"
					)
				})?
		}
		...
		result = publisher_done.changed() => {
			...
			consumer
				.routed_broadcast(&broadcast_name)
				.await
				.with_context(|| {
					format!(
						"resolving broadcast `{broadcast_name}`"
	...
	let track = broadcast
		.track(&track_name)
		.with_context(|| format!("locating track `{track_name}` in broadcast `{broadcast_name}`"))?
		.subscribe(None)
		.await
		.with_context(|| format!("subscribing to track `{track_name}`"))?;
...
async fn consume_raw_track(
	...
	loop {
		...
		while let Some(frame) = group.read_frame().await.context("reading raw MoQ frame")? {
...
fn print_encrypted_frame(payload: &[u8], raw: bool) -> Result<()> {
	...
	if raw {
		stdout.write_all(payload).context("writing raw encrypted frame")?;
