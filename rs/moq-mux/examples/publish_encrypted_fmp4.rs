...
impl Options {
    fn parse() -> Result<Self> {
        ...
        let input = args
            .next()
            .map(PathBuf::from)
            .ok_or_else(|| anyhow!("usage: publish_encrypted_fmp4 <input.mp4> [options]"))?;
                    ...
                    println!(
                        "usage: publish_encrypted_fmp4 <input.mp4> [options]\n\
                         ...
                         options:\n\
                           ...
                           --track <name>      track to subscribe to\n\
...
async fn main() -> Result<()> {
    ...
    let encoded_aead_key = env::var("MOQ_AEAD_KEY")
        .context("MOQ_AEAD_KEY must contain a 32-byte key encoded as hex or base64")?;
    ...
    let publisher_result =
        run_publisher(&origin, encrypter, &options.input, &options.broadcast_name).await;
    ...
    // Stop the background tasks after publishing and subscribing finish.
...
async fn connect_to_relay(relay_url: &str) -> Result<MoqSession> {
    let (origin, _driver) =
        moq_net::origin::Producer::new(moq_net::Hop::random().into());
...
async fn run_publisher(
    ...
    let mut importer = Import::new(broadcast, catalog.reserve()).with_encrypter(encrypter);
...
async fn run_subscriber(
    ...
    let track = broadcast
        ...
        .with_context(|| {
            format!(
                "locating track `{track_name}` in broadcast `{broadcast_name}`"
            )
        })?
        .subscribe(None)
        .await
