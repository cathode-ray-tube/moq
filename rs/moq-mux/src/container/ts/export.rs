...
impl<E: catalog::Catalog> Export<E> {
    ...
    /// Creates a decrypter factory using the supplied configuration.
    ///
    /// The factory can produce any number of decrypters with the given config.
    ...
    where
        C: Send + Sync + 'static,
        F: Fn(&C) -> Option<Decrypter> + Send + Sync + 'static,
    {
        self.decrypter_factory = Some(Box::new(move || factory(&config)));

        self
    }

    /// Creates a new decrypter from the configured factory.
    ...
    /// Pad the output with null packets to `mux_rate` bits per second,
    /// whatever the catalog records. Without this, the catalog's
    /// `mpegts.muxRate` decides, and a catalog without one leaves the output
    /// unpadded.
    ///
    ...
    pub fn with_mux_rate(mut self, mux_rate: u64) -> Self {
        if sanitize_mux_rate(mux_rate).is_none() {
            tracing::warn!(
                mux_rate,
                "ignoring invalid MPEG-TS multiplex rate override"
            );
        }
