	...
	crate::Error: From<C::Error>,
{
    ...
    ///
    /// The encrypter is retained for the lifetime of the producer so its
    /// encryption counter can advance across immediate writes and buffered
    /// flushes.
    ...
    fn write_container(&mut self, frames: &[Frame]) -> crate::Result<()> {
        ...
        match self.encrypter.as_mut() {
            ...
            None => {
                ...
                self.container.write(&mut plain, frames)?;
            }
        }
...
impl<R> Guard<'_, R> {
    ...
    pub fn commit(mut self) -> crate::Result<()> {
        let config = self
            .config
            .take()
            .expect("config is present until commit");
