...
- `moq-json` (lib): generic JSON publishing over a track. `snapshot` is lossy latest-value (RFC 7396 merge-patch deltas; consumers only get the most recent value; `Producer<T>`/`Consumer<T>`, `Guard<T>` RAII edit); `stream` is a lossless append-log (every record preserved in order); `window` is an ordered retained range with push/pop/skip events and group checkpoints. DEFLATE via `moq-flate`.
- `moq-binary` (lib): opaque binary payloads over a track: the same two modes as `moq-json`, without the JSON. `snapshot` is lossy latest-value (one self-contained group per value); `stream` is a lossless append-log (a single group, one payload per frame, sharing one DEFLATE window). Compression via `moq-flate`, framed identically to moq-json so the two agree on the wire.
...
- **Don't hand-roll `Serialize`/`Deserialize`.** A manual impl is a second copy of the type's wire shape that drifts from the fields it mirrors, and it is almost never needed. Derive it, and reach for `serde_with` for anything the derive can't express: a string-valued enum that must round-trip an unrecognized value is `derive_more::Display` + `FromStr` plus `#[serde_as(as = "DisplayFromStr")]` on the field (`catalog::Mode`, `catalog::TextFormat`), and a numeric wire field that should be a typed duration is `DurationMilliSeconds`. Write the impl by hand only when the wire shape genuinely isn't a function of the fields, and say why in a comment.
- **Errors**: `thiserror` with `#[from]` for libraries, `anyhow` (with `.context("...")`, not `.map_err(|_| anyhow!())`) for binaries. Always `#[non_exhaustive]` on public error enums (e.g. `moq-net/src/error.rs`, `moq-ffi/src/error.rs`, `moq-loc/src/lib.rs`). **Never put a dependency's error type in a public variant** unless the crate is also re-exported (`moq-hls` re-exports `reqwest`, `moq-mux` re-exports `mp4_atom`, `moq-tokio` re-exports `rustls`). Otherwise a major bump of that dependency is a silent semver break for us and forces every consumer onto the same version. Store the message instead and keep a hand-written `From` so `?` still works: `moq-tokio/src/error.rs` has the `message` helper (which flattens the `source()` chain) and the `from_message!` macro the backend modules use. When callers act on something inside the foreign error, read it at conversion time and keep it as a field of its own (`quinn::Error::Client { message, status }`), not by matching the stored error later.
...
- **A wrapper the host installed stays reachable in the dev shell.** `nix develop` prepends the pinned toolchain ahead of everything else, which shadows the Cargo shim `mbx setup` writes, so a bare `cargo` there would skip the wrapper it hits anywhere else. The `shellHook` in `flake.nix` puts it back in front, asking `mbx setup --status` where it lives rather than assuming a path. It restores only what the host already had, and it is skipped under `CI`, where the job has to build in the `target/` its workflow restored rather than a machine-wide managed one. Compilation caching itself never depended on this: `mbx setup` also writes `build.rustc-wrapper` into `~/.cargo/config.toml`, which every Cargo reads. The shim is what adds the managed `target/`.
